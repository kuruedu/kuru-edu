...
public class QuizGeneratorMulti : MonoBehaviour {
	...
	public void Answering(int ans){
		if (ans == 1) {
			if (ans == correct) {
				//benar
				print("benar");
				if (this.GetComponent<PlayerNetworked> ().PC.isfinal) {
					...
					this.GetComponent<PlayerNetworked> ().PC.moveNextBenarJawab ();
				}
				this.GetComponent<PlayerNetworked> ().PC.isQuiz = false;
				//QuizTime.SetActive (false);
				...
				//salah
				print("salah");
				//this.GetComponent<PlayerNetworked> ().PC.currentPos -= this.GetComponent<PlayerNetworked> ().PC.getDadu;
				this.GetComponent<PlayerNetworked> ().PC.moveBackSalahJawab();
				this.GetComponent<PlayerNetworked> ().PC.isQuiz = false;
				//QuizTime.SetActive (false);
				StartCoroutine(hideQuiz());
				QuizTime.GetComponent<Animator> ().SetBool ("QuizTime", false);
				...
				if (this.GetComponent<PlayerNetworked> ().PC.isfinal) {
					this.GetComponent<PlayerNetworked> ().PC.isfinal = false;
				}
